...
async function getAwsData(): Promise<AwsInstance[]> {
  ...
  const response = await fetch("https://sheetdb.io/api/v1/ddtqulvvu5lba", { next: { revalidate: 3600 } });
...
async function getAzureData(): Promise<AzureInstance[]> {
  // Fetch data from your API here.
  const response = await fetch("https://sheetdb.io/api/v1/nl5lyk7jf10lm", { next: { revalidate: 3600 } });
  const data = await response.json();
  return data;
}
